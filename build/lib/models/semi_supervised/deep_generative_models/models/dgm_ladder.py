...
class DeepGenerativeModel(VAE):
        ...
        if not only_na:
            with torch.no_grad():
                ...
                hparams_string = "/".join(["num_elements" + str(self.num_elements), "n_flows" + str(self.n_flows),
                                           "z_dim" + str(self.z_dim_last), "a_dim" + str(self.a_dim),
                                           "lr" + str(self.lr),
                    ...
                    if show_lda != 0 and epoch % show_lda == 0 and epoch != 0:
                        try:
                            ordination2d(df, "lda", epoch=self.epoch, images_folder_path=images_path,
                                         dataset_name=self.dataset_name, a=0.5,
                                         verbose=0, info="generated", n=self.num_classes)
                        except:
                            if verbose > 0:
                                print("NO lda")
        else:
            ...
            create_missing_folders(images_path)
            if verbose > 0:
                print("GENERATING N/A IMAGES AT", images_path)
            # self.eval()
            rand_z = Variable(torch.randn(n * self.num_classes, self.z_dim))
    ...
    def run(self, n_epochs, auxiliary, mc=1, iw=1, lambda1=0., lambda2=0., verbose=1, show_progress=0,
        ...
        hparams_string = "/".join(["num_elements"+str(self.num_elements), "n_flows"+str(self.n_flows),
                                   "z_dim"+str(self.z_dim_last), "a_dim"+str(self.a_dim), "lr"+str(self.lr),
                                   "ladder"+str(self.ladder), 'generate_extra:'+str(generate_extra_class),self.flavour])
                ...
                for (x, y), (u, _) in zip(cycle(self.train_loader), self.train_loader_unlabelled):
                    ...
                    if self.epoch > generate_extra_class:
                        ...
                        try:
                            ...
                            exit("Whoops, dgm line 441")
                        x = torch.cat([x, x_na], 0)
                        y = torch.cat([y, y_na])
                        del x_na, y_na

                    L, rec, z_q = elbo(x.float(), y.float(), self.valid_bool)
                    ...
                    U, _, _ = elbo(u.float(), y=None, valid_bool=self.valid_bool)
    ...
    #
                ...
                ordination2d(data_frame_train, "pca", self.images_path + "/pca/",
                             self.dataset_name, epoch, targets=cat_targets)
                ordination2d(data_frame_train, "lda", self.images_path + "/lda/",
                             self.dataset_name, epoch, targets=cat_targets)
                ...
                with torch.no_grad():
                    ...
                    for x, y in self.valid_loader:
                        ...
                        L, rec, self.zs_test = elbo(x.float(), y, self.valid_bool, valid=True)
                        self.zs_test_targets = [np.where(r==1)[0][0] for r in y.detach().cpu().numpy()]

                        U, _, _ = elbo(x.float(), y=None, valid_bool=self.valid_bool, valid=True)
