...
import numpy as np
from models.semi_supervised.deep_generative_models.layers.flow import NormalizingFlows as nf
from models.semi_supervised.deep_generative_models.layers.flow import HouseholderFlow as hf
...
class AuxiliaryDeepGenerativeModel(DeepGenerativeModel):
    def __init__(self, flow_type, z_dims, h_dims, n_flows, a_dim, num_elements, labels_per_class,
                 dropout=0.5, is_pruning=False, gt_input=-100, use_conv=False):
        """
        ...
        super(AuxiliaryDeepGenerativeModel, self).__init__(flow_type=flow_type, z_dims=z_dims, h_dims=h_dims,
                                                           ...
                                                           is_pruning=is_pruning, gt_input=gt_input,
                                                           supervised="semi", labels_per_class=labels_per_class)
    ...
    def set_adgm_layers(self, h_dims, input_shape, load_vae=False, use_conv_classifier=False,
        ...
        if use_conv_classifier:
            ...
            self.set_dgm_layers(input_shape=input_shape, num_classes=self.num_classes)
            self.classifier = MLP(self.input_size, self.input_shape, self.indices_names, h_dims, self.num_classes,
                                  a_dim=self.a_dim, is_pruning=self.is_pruning, gt_input=self.gt_input,
        ...
        if self.flow_flavour in [nf, hf]:
            self.add_flow_auxiliary(self.flow_flavour(in_features=[self.a_dim], n_flows=self.n_flows,
                                                      h_last_dim=h_dims[-1], auxiliary=True))
            self.add_flow(self.flow_flavour(in_features=[self.z_dim], n_flows=self.n_flows,
                                                      h_last_dim=h_dims[-1], auxiliary=False))

        elif self.flow_flavour is not "Vanilla":
            self.add_flow_auxiliary(self.flow_flavour(in_features=[self.a_dim], n_flows=self.n_flows,
                                                      h_last_dim=h_dims[-1], auxiliary=True, flow_flavour=self.flavour))
    ...
    def set_conv_adgm_layers(self, hs_ae, h_dims, planes_ae, kernels_ae, padding_ae,
        ...
        if use_conv_classifier:
            ...
            self.set_dgm_layers(input_shape=self.input_shape)
            self.classifier = MLP(self.input_size, self.input_shape, h_dims, self.num_classes,
    ...
    def forward(self, x, y, auxiliary=True):
        """
        ...
        a_kl = self._kld(q_a, q_param=(q_a_mu, q_a_log_var), i=0, h_last=h_a0, p_param=(p_a_mu, p_a_log_var),
                         auxiliary=True)
        z_kl = self._kld(z, q_param=(z_mu, z_log_var), i=0, h_last=h, auxiliary=False)

        self.kl_divergence = a_kl + z_kl
        del a_kl, z_kl, p_a, p_a_mu, p_a_log_var, h_a, x, z_mu, z_log_var, q_a_mu, q_a_log_var, h_a0, h
        return x_mean, z, q_a
