...
from models.utils.utils import create_missing_folders, make_classes
import torch
import torch.nn as nn
from models.utils.data_split import validation_split
...
class NeuralNet(nn.Module):
    ...
    def set_configs(self, home_path, dataset_name, extra_class, num_classes, results_folder="results", data_folder = "data", destination_folder="annleukemia",
    ...
    def set_data(self, has_unlabelled_samples, ratio_training=0.8, ratio_valid=0.1, is_example=False,
                 is_split=True, ignore_training_inputs=0, is_custom_data=False):
        ...
        self.y_train = to_categorical([np.int(classe) for classe in self.classes_train], self.num_classes)
        self.y_valid = to_categorical([np.int(classe) for classe in self.classes_valid], self.num_classes)
        try:
        ...
        self.y_test = to_categorical([np.int(classe) for classe in self.classes_test], self.num_classes)
        if not is_example and not is_custom_data:
            ...
            self.make_loaders(train, valid, test, self.labels_per_class, unlabelled_train_ds=unlabelled_train,
                              unlabelled_samples=has_unlabelled_samples)


    def load_local_dataset(self, root_train, root_valid, root_test, n_classes, batch_size=128,
        ...
        self.hparams_string += "/".join([self.hparams_string, "labels_per_class"+str(self.labels_per_class),
        ...
        self.make_loaders(train_ds=train_ds, valid_ds=valid_ds, test_ds=test_ds, labels_per_class=self.labels_per_class,
                          unlabelled_train_ds=None, unlabelled_samples=unlabelled_train_ds)


    def load_example_dataset(self, dataset, valid_prop=0.5, batch_size=128, labels_per_class=-1,
                             unlabelled_train_ds=None, normalize=True, mu=0.1307, var=0.3081, unlabelled_samples=True,
                             na_folder=None):
        from models.semi_supervised.utils.utils import one_hot
        import torchvision.transforms as transforms

        if unlabelled_samples is False:
            unlabelled_samples = []
    ...
    def make_loaders(self, train_ds, valid_ds, test_ds, labels_per_class, unlabelled_train_ds,
                     unlabelled_samples=False, na_folder=None):
        from functools import reduce
        from operator import __or__
        from torch.utils.data.sampler import SubsetRandomSampler
        dataset_folder = dset.ImageFolder
        import torchvision.transforms as transforms
        flatten = lambda x: \
            transforms.Compose([transforms.ToTensor()])(x).view(-1)
        if unlabelled_samples is False:
        ...
        if na_folder is not None:
            import os
            na_ds = dataset_folder(root=os.getcwd() + "/na_images", transform=flatten)
            na_loader = torch.utils.data.DataLoader(
                        dataset=na_ds,
                        batch_size=self.batch_size,
                        shuffle=True,
                        num_workers=0,
        ...
        #na_loader.dataset.samples
        #self.train_loader.dataset.train_data
        #self.train_loader.dataset.train_labels
    ...
    def kill_dead_neurites(self, threshold=1e-8, sparse_tensor=False, verbose=True):
        """
        This function puts neurites very close to 0, to 0.0.
        With L1 penalty, parameters might be put very close to 0; but it won't ever be exactly 0.
        This function kills neurites that have values less then a threshold value
        :return:
        """
        dead_neurites = 0
        total_neurites = 0
        ...
        if not sparse_tensor:
            for m in self.modules():
                if isinstance(m, nn.Linear):
                    m.bias.data = torch.Tensor([0 for _ in range(len(m.bias.data))]).cuda()
                    data = m.weight.data
                    total_neurites += np.multiply(data.shape[0], data.shape[1])
                    for i in range(data.shape[0]):
                        for j in range(data.shape[1]):
                            if abs(float(data[i][j])) < min:
                                min = abs(float(data[i][j]))
                            if abs(float(data[i][j])) < threshold and float(data[i][j]) != 0.0:
                                data[i][j] = 0.0
                                dead_neurites += 1
                                self.dead_neurites_count += 1
        else:
            exit("Not implemented yet")

        if verbose:
            print("Number of dead neurites:", self.dead_neurites_count, "/", total_neurites)
            print("Min: ", min)
            print("Number of neurites killed during epoch ", self.epoch, ":", dead_neurites)
