...
from models.utils import log_standard_categorical
import numpy as np
...
class SVI(nn.Module):
    ...
    def forward(self, x, y, valid_bool, valid=False):
        ...
        if not self.model.sylvester_flow:
            reconstruction, z_q, a_q = self.model(xs, y=ys)
        else:
            if not self.auxiliary:
                reconstruction = self.model.run_sylvester(xs, k=0, y=ys, auxiliary=False)
            else:
                reconstruction, z_q, a_q = self.model.run_sylvester_dgm(xs, k=0, y=ys, auxiliary=True)
        try:
        ...
        if is_labelled:
            ...
            return -torch.mean(L), reconstruction, z_q, a_q
        ...
        # Equivalent to -U(x)
        U = L + H
        del L, H, reconstruction, elbo
        return -torch.mean(U), None, z_q, a_q