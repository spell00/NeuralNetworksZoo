...
from models.semi_supervised.deep_generative_models.layers.flow import Sylvester, TriangularSylvester
from models.generative.autoencoders.vae.vae import VariationalAutoencoder


class SylvesterVAE(VariationalAutoencoder):
    def __init__(self, flavour, z_dims, h_dims, auxiliary, n_flows, a_dim, kernels=None,
                 padding=None, num_elements=None, is_pruning=False,dropout=0.5):
        super(SylvesterVAE, self).__init__(flavour, z_dims, h_dims, auxiliary, n_flows, kernels,
                                           padding, is_pruning=is_pruning, dropout=dropout)
        ...
        if self.flow_type == "o-sylvester":
            ...
            self.amor_d = nn.Linear(self.h_dims[-1], self.n_flows * self.num_elements * self.num_elements)
            self.amor_diag1 = nn.Sequential(
                nn.Linear(self.h_dims[-1], self.n_flows * self.num_elements),
            ...
            self.amor_b = nn.Linear(self.h_dims[-1], self.n_flows * self.num_elements)
            if len(self.z_dims) > 1:
                self.amor_q = [nn.Linear(self.h_dims[-1], self.n_flows * z_dim * self.num_elements) for z_dim in
                           self.z_dims]
            else:
                self.amor_q = nn.Linear(self.h_dims[-1], self.n_flows * self.z_dim_last * self.num_elements)

            if self.auxiliary:
                if len(self.a_dims) > 1:
                    self.amor_q_a = [nn.Linear(self.h_dims[-1], self.n_flows * a_dim * self.num_elements) for a_dim in
                                 self.a_dims]
                else:
                    try:
                        self.amor_q_a = nn.Linear(self.h_dims[-1], self.n_flows * self.a_dims[0] * self.num_elements)
                    except:
                        self.amor_q_a = nn.Linear(self.h_dims[-1], self.n_flows * self.a_dims[0][0] * self.num_elements)
            ...
            self.amor_d = nn.Linear(self.h_dims[-1], self.n_flows * self.z_dim_last * self.z_dim_last)
            self.amor_diag1 = nn.Sequential(
                nn.Linear(self.h_dims[-1], self.n_flows * self.z_dim_last),
                self.diag_activation
            )
            self.amor_diag2 = nn.Sequential(
                nn.Linear(self.h_dims[-1], self.n_flows * self.z_dim_last),
            ...
            self.amor_b = nn.Linear(self.h_dims[-1], self.n_flows * self.z_dim_last)
            ...
            if self.auxiliary:
                ...
                self.amor_d_a = nn.Linear(self.h_dims[-1], self.n_flows * self.a_dim * self.a_dim)
                self.amor_diag1_a = nn.Sequential(
                    nn.Linear(self.h_dims[-1], self.n_flows * self.a_dim),
                    self.diag_activation
                )
                ...
                self.amor_b_a = nn.Linear(self.h_dims[-1], self.n_flows * self.a_dim)
                if self.flow_type == "h-sylvester":
                    self.amor_q_a = [nn.Linear(self.h_dims[-1], self.n_flows * a_dim * self.num_elements) for a_dim in
                               self.a_dims]
    ...
    def batch_construct_orthogonal(self, q, auxiliary, i=-1):
        ...
        dim0 = amat.size(0)
        amat = amat.resize(dim0, last, self.num_elements)
    ...
    def encode(self, x, y, a, auxiliary, i=0):
        ...
        if auxiliary and self.flavour != "o-sylvester":
            if self.flavour != "t-sylvester":
        ...
        if self.flavour == "o-sylvester" and auxiliary:
            amor_q = self.amor_q_a
            last = self.a_dim
        ...
        batch_size = x.size(0)
        if auxiliary:
            (z_q, mean_z, var_z), h = self.aux_encoder(x, y=torch.FloatTensor([]).cuda(), a=torch.FloatTensor([]).cuda(),
                                                       input_shape=self.input_shape)
        else:
            n = int(torch.prod(torch.Tensor(self.input_shape)))
            x1 = x[:, :n]
            try:
                (z_q, mean_z, var_z), h = self.encoder(x1, y=y, a=a, input_shape=self.input_shape)
            except:
                z, h = self.encoder(x1, y=y, a=a, input_shape=self.input_shape)
                if self.flavour == "o-sylvester":
                    mean_z, var_z = torch.mean(torch.Tensor(z), 0).view(-1, 1).cuda(), torch.var(torch.Tensor(z), 0).view(-1, 1).cuda()
                    z1 = z
                else:
        ...
        if self.flow_type == "o-sylvester":
            full_d = full_d.resize(batch_size, self.num_elements, self.num_elements, self.n_flows)
            diag1 = diag1.resize(batch_size, self.num_elements, self.n_flows)
            diag2 = diag2.resize(batch_size, self.num_elements, self.n_flows)
            b = b.resize(batch_size, 1, self.num_elements, self.n_flows)
            q = amor_q(h)
        elif self.flow_type == "h-sylvester" or self.flow_type == "t-sylvester":
            full_d = full_d.resize(batch_size, last, last, self.n_flows)
            diag1 = diag1.resize(batch_size, last, self.n_flows)
            diag2 = diag2.resize(batch_size, last, self.n_flows)
            b = b.resize(batch_size, 1, last, self.n_flows)
    ...
    def forward(self, x, y=torch.Tensor([]).cuda(), a=torch.Tensor([]).cuda(), k=0, auxiliary=False):
        ...
        for i in range(self.n_flows):
            ...
            if self.flow_type in ["o-sylvester"]:
                z_k, log_det_jacobian = flow_k(z[i], r1[:, :, :, i], r2[:, :, :, i], q_ortho[i, :, :, :], b[:, :, :, i])
                ...
                if k % 2 == 1:
                    ...
                    permute_z = None
                z_k, log_det_jacobian = flow_k(z[i], r1[:, :, :, i], r2[:, :, :, i], b[:, :, :, i], permute_z, sum_ldj=True)
            else:
        ...
        log_q_z0 = log_gaussian(z[-1], z_mu, log_var=z_var) - self.log_det_j
    ...
    def run_sylvester(self, x, y=torch.Tensor([]).cuda(), a=torch.Tensor([]).cuda(), k=0, auxiliary=True, exception=False):
        """
        Forward pass with orthogonal sylvester flows for the transformation z_0 -> z_1 -> ... -> z_k.
        Log determinant is computed as log_det_j = N E_q_z0[\sum_k log |det dz_k/dz_k-1| ].
        """
        ...
        for i in range(self.n_flows):
            ...
            if self.flow_type in ["o-sylvester"]:
                ...
                if k % 2 == 1:
                    ...
                    permute_z = None
                z_k, log_det_jacobian = flow_k(zk=z[i], r1=r1[:, :, :, i], r2=r2[:, :, :, i], b=b[:, :, :, i], permute_z=permute_z,
