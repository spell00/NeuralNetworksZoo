...
class Encoder(nn.Module):
    def __init__(self, input_size, h_dim, z_dim, num_classes, y_dim, sample_layer=GaussianSample, a_dim=0):
        ...
        try:
            ...
            neurons[0] = int(neurons[0]) # TODO correct problem at source
            linear_layers = [nn.Linear(neurons[i-1], neurons[i]) for i in range(1, len(neurons))]
        ...
        try:
            ...
            self.sample = sample_layer(h_dim[-1], z_dim[0])
        self.cuda()
        self.Gate = Gate()
...
class VariationalAutoencoder(AE):
    def __init__(self, flavour, z_dims, h_dims, planes=None, pooling_layers=None, kernels=None, padding=None, auxiliary=False, n_flows=0,
                 a_dim=0, dropout=0.5, num_elements=None, is_pruning=False):
        """
        ...
        if n_flows > 0 and not self.sylvester_flow and self.flow_flavour is not None and self.flow_flavour != "Vanilla":
