...
def __main__():
    ...
    mlp = MLP(input_size=784, input_shape=(1, 28, 28), indices_names=list(range(784)),
              ...
              gt_input=gt_input, gt=gt, is_pruning=is_pruning, dropout=dropout, labels_per_class=-1,
    ...
    mlp.set_data(is_example=True, extra_class=True, ignore_training_inputs=3)

    mlp.cuda()
    # dgm.vae.generate_random(False, batch_size, z1_size, [1, 28, 28])
    mlp.run(n_epochs, start_pruning=5)
